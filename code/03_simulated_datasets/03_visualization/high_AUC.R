...
y2 <- as.data.frame.table(data2$y) %>%
...
gof1 <- readRDS(here('data_outputs',
                        '03_simulated',
                        '04_posterior_summaries',
                        'mod1high_GOFmodel.RDS'))

yrep1 <- as.data.frame.table(gof1$sims.list$yrep) %>%
...
p1 <- as.data.frame.table(gof1$sims.list$p) %>%
  rename('iteration' = "Var1",
         "individual" = "Var2",
         "site" = "Var3",
         'p' = "Freq")%>%
...
gof2 <- readRDS(here('data_outputs',
                        '03_simulated',
                        '04_posterior_summaries',
                        'mod2high_GOFmodel.RDS'))

yrep2 <- as.data.frame.table(gof2$sims.list$yrep) %>%
  rename('iteration' = "Var1",
         "individual" = "Var2",
         'interval' = "Var3",
         "site" = "Var4",
...
p2 <- as.data.frame.table(gof2$sims.list$p.int) %>%
  rename('iteration' = "Var1",
         "individual" = "Var2",
         'interval' = "Var3",
         "site" = "Var4",
...
gof3 <- readRDS(here('data_outputs',
                             '03_simulated',
                             '04_posterior_summaries',
                             'mod3high_GOFmodel.RDS'))

yrep3.1 <- as.data.frame.table(gof3$sims.list$yrep_1) %>%
  rename('iteration' = "Var1",
         "individual" = "Var2",
         "site" = "Var3",
         'yrep' = "Freq")
...
yrep3.2 <- as.data.frame.table(gof3$sims.list$yrep_2) %>%
  rename('iteration' = "Var1",
         "individual" = "Var2",
         "site" = "Var3",
         'yrep' = "Freq") %>%
  anti_join(yrep3.1id, by = c("iteration", "individual", "site"))

yrep3 <- bind_rows(yrep3.1, yrep3.2)%>%
  mutate(site = as.numeric(site),
         individual = as.numeric(individual),
         iteration = as.numeric(iteration))

p3.1 <- as.data.frame.table(gof3$sims.list$pi1) %>%
  rename('iteration' = "Var1",
         "individual" = "Var2",
         "site" = "Var3",
         'p' = "Freq")

p3.2 <- as.data.frame.table(gof3$sims.list$pi2) %>%
  rename('iteration' = "Var1",
         "individual" = "Var2",
         "site" = "Var3",
         'p' = "Freq") %>%
...
resp2 <- as.vector(y2$y)

mod2_AUC <- rep(NA, iteration.num)

for(i in 1:iteration.num){
  mod2_AUC[i] <- AUC_JAGS2(p2, 
                           iteration.num = i, 
                           resp = resp2)
}
...
resp3 <- as.vector(y3$y)

mod3_AUC <- rep(NA, iteration.num)

for(i in 1:iteration.num){
  mod3_AUC[i] <- AUC_JAGS2(p3, 
                           iteration.num = i, 
                           resp = resp3)
}
...
conf_df1 <- yrep1 %>%
  left_join(y1, by = c("individual", "site"))

conf_df2 <- yrep2 %>%
  left_join(y2, by = c("individual", 
                          "site",
                          'interval'))

conf_df3 <- yrep3 %>%
  left_join(y3, by = c("individual", "site"))
