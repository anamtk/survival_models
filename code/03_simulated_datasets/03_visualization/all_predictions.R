...
stats_df <- bind_rows(stats_list, 
  ...
  mutate(mod = str_sub(model, 1, 4),
         var = case_when(str_detect(model, "low") ~ "low",
                         str_detect(model, "med") ~ "med",
                         str_detect(model, 'high') ~ "high",
                         TRUE ~ NA_character_),
         interval = case_when(str_detect(model, "short") ~ "short",
                              str_detect(model, "long") ~ "long",
                              TRUE ~ NA_character_)) %>%
  mutate(mod = str_sub(model, 1, 4),
         var = case_when(str_detect(model, "low") ~ "low",
                         str_detect(model, "med") ~ "med",
