...
#prepping data with one of the "total"
#datasets for the exposure "null" model
exposure_model_dataprep_fun <- function(tot.df){
...
interval_model_dataprep_fun <- function(int.df){
  ...
  n.int <- max(int.df$interval) #max number of intervals per individual
  n.data <- 100 #number of datasets
  
  ID <- int.df$ID #define a vector of ids
  interval <- int.df$interval #vector of intervals
  Dataset <- int.df$Dataset #vector of datasets
  ...
  for(i in 1:dim(int.df)[1]){
    y[ID[i], interval[i], Dataset[i]] <- int.df[i, 4] #column 5 is y data
  }
  ...
  for(i in 1:dim(int.df)[1]){
    t[ID[i], interval[i], Dataset[i]] <- int.df[i, 5]
  }
...
custom_model_dataprep_fun <- function(day.df){
  ...
  n.ind <- 30 #individuals
  n.int <- max(day2$interval) #max number of intervals per individual
  n.dy <- max(day2$day)
  n.data <- 100 #number of datasets
  ...
  day <- day2$day #vector of days in interval
  datset <- day2$Dataset
  ...
  #fill array based on id, interval, day, and dataset for each row
  for(i in 1:dim(day2)[1]){
    x[day[i], datset[i]] <- day2[i, 3] 
  }
