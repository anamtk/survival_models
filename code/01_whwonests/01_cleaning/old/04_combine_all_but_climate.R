...
nests3 <- visits2 %>%
  ...
  distinct(Nest_ID, Visit_date, No_eggs,        
           No_young, Stage, Year_located,   
           Project_ID, Transect_ID, Trt_cat,        
           Fate, UTM_datum_zone, UTM_N,          
           UTM_E, Nest_Ht, Decay_class,    
           Tree_Snag_Log, Tree_sp, Tree_ht,        
           DBH, Orientation, Aspect,         
           Slope, Initiation_date, NoFL_uncert,    
           NoFL_cert, Trees_2550, Trees_50,       
           pPIPO, PIPO_2550, PIPO_50,        
           Fate_cat, Peeped) %>%
  group_by(Nest_ID) %>%
  filter(!(Visit_date == min(as_date(Visit_date)) & No_eggs == 999)) %>%
  filter(!(Nest_ID == 'EMMAOR_NPAWT1-03A_2021' & 
           Visit_date == '2021-07-09' & Stage !=F)) %>%
  ungroup() 
  
t <- nests3 %>%
    group_by(Nest_ID) %>%
    arrange(Visit_date, No_eggs) %>%
    filter(duplicated(Visit_date)) %>%
  ungroup()
...
nests6 <- nests5 %>%
  dplyr::select(Nest_ID, Visit_date,No_eggs, No_young,
                ...
                pPIPO, Fate_cat, Peeped,  Julian_end,
                Julian_start, 
