#need:
#yearly data for each plot for yearly covariates
#summarising that yearly data per survey interval
#summarizing across the whole lifetime of a tree
# Load packages -----------------------------------------------------------
...
trees2 %>%
  group_by(CoreID) %>%
  mutate(fate = case_when(any(response == "Dead") ~ "0",
                          TRUE ~ "1")) %>%
  distinct(CoreID, fate) %>%
  group_by(fate) %>%
  tally()
